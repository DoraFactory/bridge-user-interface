...
export const MnemonicExportDialog = ({ setIsOpen }: DialogProps) => {
  ...
  const title = {
    [MnemonicExportStep.AcknowledgeRisk]: stringGetter({
      key: STRING_KEYS.REVEAL_SECRET_PHRASE,
    }),
    [MnemonicExportStep.DisplayMnemonic]: stringGetter({
      key: STRING_KEYS.EXPORT_SECRET_PHRASE,
    }),
  }[currentStep];
      ...
      <>
        <Styled.WaitingSpan>
          <Styled.CautionIconContainer>
            <Icon iconName={IconName.CautionCircleStroke} />
          </Styled.CautionIconContainer>
    ...
    [MnemonicExportStep.DisplayMnemonic]: (
      <>
        ...
        <Styled.RevealControls>
          <span>
            {stringGetter({
              key: isShowing ? STRING_KEYS.NOT_READY : STRING_KEYS.READY,
            })}
          </span>
          <ToggleButton
